"""
allreduce.py — benchmarks dist.all_reduce using the mini_nccl backend.

Sweeps a range of per-rank tensor sizes, times the all_reduce collective over
many iterations on each, and reports latency and algorithmic bandwidth.

"Algorithm bandwidth" here follows the NCCL convention for all-reduce
(ring algorithm):
    bus_bytes = input_bytes * 2 * (world_size - 1) / world_size
i.e. the bytes that must cross the interconnect per rank (reduce-scatter
phase + all-gather phase), independent of how the collective is implemented.

Usage:
    uv run python allreduce.py [--backend {mini_nccl,nccl}]
...
def worker(rank: int, world_size: int, store_file: str, backend: str) -> None:
    ...
    if rank == 0:
        print(
            f"all_reduce benchmark — backend={backend}, world_size={world_size}, "
    ...
    for nbytes in SIZES_BYTES:
        ...
        # all_reduce is in-place: one tensor per rank, summed across ranks.
        tensor = torch.full(
        ...
        if rank == 0:
            # NCCL-style algorithm bandwidth for ring all-reduce: each rank
            # both sends and receives ~2*(n-1)/n of its data over the wire.
            bus_bytes = nbytes * 2 * (world_size - 1) / world_size
...
def main() -> None:
    ...
    if world_size < 2:
        raise SystemExit(
            f"Found {world_size} GPU(s) — need at least 2 to benchmark all-reduce."
        )
